...
public class MemberOpenServiceImpl implements MemberOpenService {
    ...
    public ApiResp<Void> edit(EditMemberOpenReq req) {
        ...
        updateDao.set(E_MemberOpen.lastUpdateTime, new Date());
    ...
    public ApiResp<Void> delete(DeleteMemberOpenReq req) {
        ...
        boolean r = jpaDao.deleteFrom(MemberOpen.class).appendByQueryObj(req).delete() > 0;
        if (!r) {
    ...
    public ApiResp<Boolean> checkBindOpen(CheckBindOpenReq req) {
        ...
        wxOpenTypes.add(OpenType.WE_CHAT);
        wxOpenTypes.add(OpenType.WE_CHAT_OPEN);
        wxOpenTypes.add(OpenType.WE_CHAT_MA);
        SelectDao<MemberOpen> selectDao = jpaDao.selectFrom(MemberOpen.class)
                .appendWhere(req.getMemberId() != null, E_MemberOpen.memberId + " = :?", req.getMemberId())
                ...
                .appendWhere(StringUtils.hasText(req.getOpenId()), E_MemberOpen.openId+ " = :?", req.getOpenId())
                .appendWhere(StringUtils.hasText(req.getUnionId()), E_MemberOpen.unionId+ " = :?", req.getUnionId());
