...
public class RoleServiceImpl implements RoleService {
    ...
    public ApiResp<Long> createRole(CreateRoleReq req) {
        ...
        oakRole.setEnabled(true);
        oakRole.setOrderCode(req.getOrderCode() == null ? 0: req.getOrderCode() );

        if (req.getPermissionIds().length > 0) {
    ...
    @Caching(
            evict = {
                    ...
                    @CacheEvict(
                            value = ROLE_CACHE_NAME,
                            key = "#req.name",
                            condition = "#req.name!=null and #result.success"
                    ),
                    @CacheEvict(
    ...
    @Caching(
            evict = {
                    @CacheEvict(
                            value = ROLE_CACHE_NAME,
                            key = "#req.id",
                            condition = "#req.id!=null and #result.success"
                    ),
                    @CacheEvict(
                            value = ROLE_CACHE_NAME,
                            key = "#req.name",
    ...
    @Caching(
            cacheable = {
                    @Cacheable(
                            value = ROLE_CACHE_NAME,
                            key = "#req.id",
                            condition = "#req.id!=null and #req.fetchPermission",
                            unless = "#result.empty"),
                    @Cacheable(
                            value = ROLE_CACHE_NAME,
                            key = "#req.name",
                            condition = "#req.name!=null and #req.fetchPermission",
                            unless = "#result.empty")
            }
    )
